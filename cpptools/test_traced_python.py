...
def main():
    ...
    for i, image_f in enumerate(images_files):
        ...
        tag_k,ind_k,val_k,_,_ = model(fimage)
        tag_k,ind_k,val_k =tag_k.cpu().detach().numpy(),ind_k.cpu().detach().numpy(),val_k.cpu().detach().numpy() 
        # size at scale 1.0
        grouped, scores = parser.parsev1(tag_k,ind_k,val_k)
        #final_results nx17x5  5(x,y,scores,tags0,tags1,...)
        final_results = get_final_predsv1(grouped)
        ...
        if True:
            ...
            save_valid_image(image, final_results[:1], prefix, dataset="COCO",color=(0,255,0))
